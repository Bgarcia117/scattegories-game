...
//   The function passed into the "useEffect" hook/function runs whenever the "tine" state value is updated (NOTE: That also includes when the page first loads)
  useEffect(() => {
    // The code inside this if statement wil NOT run when the value of the "time" state variable is equal to the amount of time (AKA Seconds) set as the starting amound for the countdown timer. 
    if(time !== startingTime){
      ...
      setTimeout(() => {
        // Checking to make sure that time has not run out (AKA Checking to see if time is equal to zero)
        if(time > 0){  
        ...
        //   Hides the list item text/content on all the categories (AKA List items) by adding the ".hide" CSS class to all of them
          setHide(true);
        //   The alert is inside another setTimeout so that it will NOT run/display before the text/content is hidden for the categories (AKA List items)
          setTimeout(() => {
            alert('TIME IS UP!!!');
            // After the alert is complete (AKA The user clicks "OK")
            setHide(false);
          }, 100);
        }
      }, 1000);
